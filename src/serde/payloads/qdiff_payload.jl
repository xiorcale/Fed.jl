...
Apply both quantization and diff-deduplication to the weights before
serialization in order to compress the payload. Quantization is a lossy process,
thus a loss of information is to be expected after deserialization.
"""
...
Serialize `weights` with the `QuantizedDedupPayloadSerde` where quantization 
...
Deserialize `data` with the `QuantizedDedupPayloadSerde` where regeneration of
