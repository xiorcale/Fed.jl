...
Apply quantization to the weights before serialization in order to reduce the
payload size. This process is lossy, thus a loss of information is to be
...
Serialize `weights` with the `QuantizedPayloadSerde` where quantization is 
